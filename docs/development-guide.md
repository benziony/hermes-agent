<!-- Retained upstream development manual, relocated from AGENTS.md. -->

Repository paths in this reference are relative to the repository root unless a section says otherwise. The root [AGENTS.md](../AGENTS.md) is the entrypoint and routes each task to the relevant complete sections below. These sections remain operative guidance; relocation does not retire upstream policy. Examples do not grant permission to discard work, publish, deploy, or change user state.
